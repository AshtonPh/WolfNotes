...
function addUser(userData){ // add a user
  ...
  let email = "";
  return db.query(
    'INSERT INTO users (userID, userName, avatar, salt, passwordHash) VALUES (NULL, ?, ?, ?, ?)',
    [userData.username, `https://robohash.org/${userData.username}.png?size=64x64&set=set1`, salt, key]
  ).then(({ results }) => {
